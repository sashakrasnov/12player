...
'''
API endpoints. This module uses also Redis memcached database
to make response to Telegram chatbot much faster
'''
...
def user_read_cache():
    '''Read or write current state of chatbot of the specified user

    Params:
        token (str): Telegram chatbot digital signature
        uid (int): Telegram-user id
        val (str): serialized data to set. Optional. If omitted, get operation instead
    Returns:
        str: serialized data
    '''
        ...
        if uid and now:
            try:
                ...
                try:
                    ...
                    for r in res:
                        resp.append({'d': r['dt'].strftime(DT_FORMAT), # Date
                                     'dt': r['diff'],                  # Date relative to now
                                     'utime': r['utime'],              # Unixtime
            ...
            try:
                ...
                try:
                    res = con.execute(text(
                            "SELECT `e`.*, UNIX_TIMESTAMP(`e`.`dt`) AS `e_utime`, `t`.`id` AS `tid`, `t`.`tg_id`, `t`.`t_buy`, `t`.`t_refund`, `t`.`status` AS `t_status`, `t`.`t_code`, `t`.`ts`, UNIX_TIMESTAMP(`t`.`ts`) AS `t_utime` \
                                FROM `events` AS `e` , `tickets` AS `t` \
                                WHERE `t`.`event_id` = `e`.`id` \
                                    AND `t`.`tg_id` = :i \
                                    AND DATE(`e`.`dt`) BETWEEN DATE_SUB(:ymd, INTERVAL 1 DAY) AND DATE_ADD(:ymd, INTERVAL :t DAY) \
                                ORDER BY `e`.`dt` ASC, `t`.`status` DESC LIMIT :l"), i=uid, ymd=now, t=DT_TICKETS_INTERVAL, l=lim)
